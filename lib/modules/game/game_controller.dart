...
void refillFuel(double amount) {
  ...
  if (money.value >= cost) {
    ...
    Get.snackbar('Error', 'Dinero insuficiente para recargar.', snackPosition: SnackPosition.BOTTOM);
  }
}
  ...
  if (money.value >= cost) {
    ...
    Get.snackbar('Error', 'Dinero insuficiente para la mejora.', snackPosition: SnackPosition.BOTTOM);
  }
}
...
void buyTyre() {
  ...
  if (money.value >= cost && tyres.value < maxTyres) {
    ...
    Get.snackbar('Llantas al maximo', 'Ya tienes el maximo nivel de llantas',
     snackPosition: SnackPosition.BOTTOM);
  } else {
    Get.snackbar('Error', 'Dinero insuficiente para la mejora (\$$cost).', 
    snackPosition: SnackPosition.BOTTOM);
  }
}
...
void continueGame() {
  if (fuel.value > 0) {
    ...
    Get.snackbar('Espera', 'Debes recargar combustible para continuar.', snackPosition: SnackPosition.BOTTOM);
  }
}
  ...
  void generateObstaclesForSize(double width, double height) {
    ...
    for (int i = 0; i < baseCount; i++) {
      ...
      if (band < 0.33) {
        ...
        spawnPoint = -(isVertical.value ? height : width) * (0.3 + _rand.nextDouble() * 0.7);
      } else if (band < 0.66) {
        // Cerca del área central
        spawnPoint = (isVertical.value ? height : width) * (0.1 + _rand.nextDouble() * 0.8);
      } else {
        // Más alejado hacia abajo (para sensación de profundidad / avance)
        spawnPoint = (isVertical.value ? height : width) * (1.0 + _rand.nextDouble() * 0.8);
      }
